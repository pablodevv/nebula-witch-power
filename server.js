...
// CHAVE É O PREFIXO NO SEU PROXY, VALOR É A URL DE DESTINO.
const TARGET_MAP = {
    '/': 'https://appnebula.co', // Default para o domínio principal
...
app.use(async (req, res) => {
    ...
    let proxyPrefix = ''; // O prefixo do proxy que será removido da URL local

    // Lógica para determinar o domínio de destino e reescrever o requestPath
    // Itera do prefixo mais longo para o mais curto para evitar correspondências parciais indesejadas
    const sortedPrefixes = Object.keys(TARGET_MAP).sort((a, b) => b.length - a.length);

    for (const prefix of sortedPrefixes) {
        if (prefix !== '/' && req.url.startsWith(prefix)) { // Se não for o prefixo padrão '/' e a URL começar com ele
            ...
            proxyPrefix = prefix;
            break; // Sai do loop assim que encontrar a correspondência mais específica
        }
    }
    ...
    console.log(`[PROXY ROUTING] Prefixo do Proxy Local: ${proxyPrefix === '' ? '/' : proxyPrefix}`);
    ...
    delete requestHeaders['origin']; // Remover o 'origin' header é crucial para algumas APIs.
    ...
    try {
            ...
            if (redirectLocation) {
                ...
                for (const prefix in TARGET_MAP) {
                    ...
                    if (proxiedRedirectPath.startsWith(originalTarget)) {
                        // Se o prefixo é '/', substitua a URL original pela raiz (vazio)
                        if (prefix === '/') {
                             proxiedRedirectPath = proxiedRedirectPath.replace(originalTarget, '');
                        } else {
                            // Para outros prefixos, substitua pela URI do prefixo do proxy
        ...
        // Adiciona cabeçalhos para evitar cache do navegador
